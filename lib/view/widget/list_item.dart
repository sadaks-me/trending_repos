...
class RepoListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<HomeProvider>(
      builder: (context, homeProvider, _) => Theme(
        data: ThemeData(
            dividerColor: Colors.transparent,
            iconTheme: IconThemeData(color: Colors.transparent, opacity: 0)),
        child: ClipRect(
          clipper: RectClipper(),
          clipBehavior: Clip.hardEdge,
          child: ExpansionPanelList(
            expansionCallback: (int index, bool status) {
              homeProvider.setActiveTileIndex(
                  homeProvider.activeTileIndex == i ? null : i);
            },
            children: [
              ExpansionPanel(
                isExpanded: homeProvider.activeTileIndex == i,
                canTapOnHeader: true,
                headerBuilder: (BuildContext context, bool isExpanded) =>
                    ListTile(
                  dense: isExpanded ? true : false,
                  leading: CircleAvatar(
                    minRadius: 20,
                    maxRadius: 20,
                    backgroundColor: Colors.grey.shade50,
                    backgroundImage: NetworkImage(checkIfNull(repo.avatar)),
                  ),
                  title: Text(
                ...
                body: ListTile(
                  dense: true,
                  contentPadding:
                      EdgeInsets.only(top: 0, right: 16, bottom: 16, left: 16),
                  leading: SizedBox(
                    height: 40,
                    width: 40,
                  ),
